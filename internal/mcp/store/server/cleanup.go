...
func CleanupUnboundServerSecrets(
	...
	for _, target := range targets {
		var kind mcpSecret.MCPSecretKind
		switch target.Kind {
		case SecretInputTargetStdioEnv:
			kind = mcpSecret.MCPSecretKindStdioEnv
		case SecretInputTargetHTTPHeader:
			kind = mcpSecret.MCPSecretKindHTTPHeader
		default:
			return fmt.Errorf(
		...
		ref, err := mcpSecret.NewMCPSecretRefString(
			server,
			kind,
			target.Slot,
		)
	...
	for _, declaration := range document.Extension.Install.Inputs {
		...
		ref, err := mcpSecret.NewMCPSecretRefString(
			server,
			mcpSecret.MCPSecretKindOAuthClientCredentials,
			"clientCredentials",
		)
		if err != nil {
			return err
